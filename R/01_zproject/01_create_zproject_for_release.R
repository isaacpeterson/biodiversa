...
build_zonation_params$data_dir <- "Zenodo/input_data/"
build_zonation_params$zsetup_root <- "zsetup_release"
build_zonation_params$aux_data_file <- paste0(build_zonation_params$data_dir, "biodiv_features.csv")
build_zonation_params$w_field <- "weight"
build_zonation_params$agg_weights_file <- paste0(build_zonation_params$data_dir, "aggregate_weights.csv")
build_zonation_params$ppa_raster_file <- paste0(build_zonation_params$data_dir, "prefecture.tif")

# FIXME: freswater fish may need a different condition file
build_zonation_params$condition_raster_file <- paste0(build_zonation_params$data_dir, "hii_rescaled_simple.tif")
# Hierarchical masks
build_zonation_params$hm3_raster_file <- paste0(build_zonation_params$data_dir, "PA_3_levels_simple.tif")
...
dat <- as.data.frame(readr::read_csv(build_zonation_params$aux_data_file))

# Read in aggregate weights
agg_weights <- readr::read_csv(build_zonation_params$agg_weights_file) %>% dplyr::filter(agg_category == "Aggregation") %>%
                dplyr::select(-agg_category) %>% tidyr::gather(taxon, agg_weight)
...
for (taxon in spp_data) {
  ...
  # Set weights
  wgts <- lookup_weight(dat, featurenames(variant), w_field = build_zonation_params$w_field)
  ...
  # Set weights
  wgts <- lookup_weight(dat, featurenames(variant), build_zonation_params$w_field)
  # sppweights()<- not implemented yet.
  variant_spp_data <- sppdata(variant)
  variant_spp_data$weight <- wgts
  sppdata(variant) <- variant_spp_data
